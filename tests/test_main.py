...
class MainTests(unittest.TestCase):
    ...
    def test_run_once_builds_preview_without_submitting_orders(self) -> None:
        ...
        result = run_once(
            snapshots=snapshots,
            now=datetime(2026, 4, 15, 3, 1, tzinfo=timezone.utc),
            previous_leader_symbol="ETHUSDT",
            client=FakeClient(),
            broker=broker,
            submit_orders=False,
    ...
    def test_run_once_live_uses_previous_closed_hour_as_stop_source(self) -> None:
        ...
        self.assertEqual(hour_call["start_time_ms"], 1776218400000)
        self.assertEqual(hour_call["end_time_ms"], 1776221999999)
    ...
    def test_run_once_live_falls_back_to_first_available_daily_minute_for_new_listing(self) -> None:
        ...
        result = run_once_live(
            symbols=["NEWUSDT"],
            now=datetime(2026, 4, 15, 3, 30, tzinfo=timezone.utc),
            previous_leader_symbol=None,
            client=client,
            broker=FakeBroker(),
            submit_orders=False,
        ...
        self.assertEqual(fallback_call["end_time_ms"], 1776223800000)
    ...
    def test_run_once_live_uses_current_hour_low_stop_when_price_is_below_previous_hour_low(self) -> None:
        ...
        class FakeClient:
            ...
            def fetch_klines(self, *, symbol, interval, limit, start_time_ms=None, end_time_ms=None):
                ...
                if start_time_ms == 1776218400000 and end_time_ms == 1776221999999:
                    return [[1776211200000, "0", "0", "110", "0"]]
                return [[1776222000000, "0", "0", "106", "0"]]
        ...
        result = run_once_live(
            symbols=["ETHUSDT"],
            now=datetime(2026, 4, 15, 3, 5, tzinfo=timezone.utc),
            previous_leader_symbol="BTCUSDT",
            client=FakeClient(),
            broker=FakeBroker(),
            submit_orders=False,
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            with redirect_stdout(out):
                exit_code = cli_main(
                    ...
                    now_provider=lambda: datetime(2026, 4, 15, 3, 1, tzinfo=timezone.utc),
                )
