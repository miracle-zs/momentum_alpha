...
def build_position_details(position_snapshot: dict, equity_value: object | None = None) -> list[dict]:
    ...
    for symbol, position in positions.items():
        ...
        avg_entry = weighted_sum / total_quantity if total_quantity > 0 else None
        if avg_entry is not None and avg_entry <= 0:
            avg_entry = None
        latest_price = _parse_numeric(_object_field(position, "latest_price"))
        if latest_price is not None and latest_price <= 0:
            latest_price = None
        risk = compute_position_risk(position) if avg_entry is not None else None
        ...
        if latest_price is not None:
            ...
            if avg_entry is not None:
                latest_price_decimal = Decimal(str(latest_price))
                if direction == "SHORT":
                    mtm_pnl = float((avg_entry - latest_price_decimal) * total_quantity)
                else:
                    mtm_pnl = float((latest_price_decimal - avg_entry) * total_quantity)
            ...
            if effective_stop_price is not None and latest_price is not None:
                if direction == "SHORT":
        ...
        details.append({
            ...
            "entry_price": f"{avg_entry:.2f}" if avg_entry is not None else None,
            "stop_price": str(stop_price) if stop_price is not None else None,
            "risk": f"{risk:.2f}" if risk is not None else None,
            "risk_pct_of_equity": risk_pct_of_equity,
            "leg_count": len(leg_info) if leg_info else None,
            "opened_at": opened_at,
