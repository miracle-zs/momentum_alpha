...
def render_daily_review_panel(report: dict | None) -> str:
    ...
    return (
        ...
        "<div><div class='daily-review-eyebrow'>COUNTERFACTUAL TRACKING</div><h3>被过滤的 Base，后来如果开仓会怎样？</h3></div>"
        f"<div class='daily-review-section-note'>{escape(filtered_status_note)} · A/B 规则只用开仓当时已完成的 1m 数据</div>"
        "</div>"
